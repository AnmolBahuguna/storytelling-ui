...
import { BookOpen, ListMusic, Plus, PlayCircle, Clock, PlusCircle, Check } from "lucide-react";
...
const Sidebar = ({ onPlayStory, onPlayPlaylist }) => {
  ...
  // --- NEW STATE FOR ADDING STORY TO PLAYLIST ---
  const [addingToPlaylistId, setAddingToPlaylistId] = useState(null); // Tracks which story's [+] is clicked
  ...
  // --- NEW FUNCTION: ADD STORY TO EXISTING PLAYLIST ---
  const handleAddToPlaylist = async (e, playlistId, story) => {
    e.stopPropagation(); // Prevents the story viewer from opening
    setIsAddingToPlaylist(true);
    const token = getToken();

    try {
      const response = await fetch(`${SERVER_URL}/api/playlists/${playlistId}/add/${story.id}`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` }
      });

      if (response.ok) {
        // Update the local playlist state instantly so we don't have to fetch from DB again
        setPlaylists(playlists.map(p => {
          if (p.id === playlistId) {
            // Add the story to the playlist's local array
  ...
  return (
    <aside className="w-80 h-full bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col">
      
      {/* Header & Tabs */}
      <div className="p-6 border-b border-slate-200 dark:border-slate-800">
        <h2 className="text-xl font-bold text-slate-800 dark:text-white mb-4">Library</h2>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2">
        {loading ? (
          <div className="text-center text-slate-500 text-sm mt-10">Loading...</div>
        ) : activeTab === "history" ? (
          
          // --- History View ---
          stories.length === 0 ? (
            <p className="text-center text-slate-500 text-sm mt-10">No stories yet. Go create one!</p>
          ) : (
            stories.map((story) => (
              <div key={story.id} className="flex flex-col mb-1 border-b border-slate-100 dark:border-slate-800/50 last:border-0 pb-1">
                ...
                >
                  ...
                  <div className="flex-1 overflow-hidden">
                    <h4 className="text-sm font-bold text-slate-800 dark:text-slate-200 truncate">{story.title}</h4>
                    <p className="text-xs text-slate-500 truncate">{story.created_at}</p>
                  </div>
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      onClick={(e) => {
                        ...
                        // Toggle the dropdown
                        setAddingToPlaylistId(addingToPlaylistId === story.id ? null : story.id);
                      }}
                ...
                {addingToPlaylistId === story.id && (
                  <div className="pl-14 pr-3 py-2 animate-in slide-in-from-top-2 duration-200">
                    <p className="text-[10px] font-bold text-slate-400 mb-2 uppercase tracking-wider">Save to...</p>
                    
                    {playlists.length === 0 ? (
                      <p className="text-xs text-slate-500">No playlists yet. Create one in the Playlists tab!</p>
                    ) : (
                      <div className="flex flex-col gap-1">
                        {playlists.map(p => {
                          // Check if this story is already in this playlist
                          const isAlreadyIn = p.stories && p.stories.some(s => s.id === story.id);
                          
                          return (
                            <button
                              key={p.id}
                              onClick={(e) => !isAlreadyIn && handleAddToPlaylist(e, p.id, story)}
                            ...
                            >
                              ...
                              {isAlreadyIn ? <Check size={14} className="text-emerald-500" /> : <Plus size={14} />}
                            </button>
                          )
                        })}
                      </div>
          ...
          <div>
            {!isCreatingPlaylist ? (
              ...
              <form onSubmit={handleCreatePlaylist} className="mb-4 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700">
                <input
                  type="text"
                  autoFocus
            ...
            {playlists.length === 0 && !isCreatingPlaylist ? (
              <p className="text-center text-slate-500 text-sm mt-6">You don't have any playlists yet.</p>
            ) : (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="text-sm font-bold text-slate-800 dark:text-slate-200">{playlist.name}</h4>
                        <p className="text-xs text-slate-500">{playlist.stories ? playlist.stories.length : 0} stories</p>
                      </div>
                    </div>
                    {playlist.stories && playlist.stories.length > 0 && (
                      <button 
                        onClick={() => onPlayPlaylist(playlist)}
                        className="p-2 bg-blue-600 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity hover:bg-blue-700 shadow-md"
                      >
